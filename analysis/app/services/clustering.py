# analysis/app/services/clustering.py
...
def compute_peer_cluster_v1(req: PeerClusterRequest) -> PeerClusterResponse:
    ...
    # -------------------------
    anchor_dates = _np_dates(anchor_ps.dates)
    common = set(anchor_dates.tolist())

    for p in selected:
        ps = price_map.get(p.stock_code)
        if ps is None:
            continue
        common &= set(_np_dates(ps.dates).tolist())
    ...
    for p in selected:
        ...
        if ps is None:
            ...
            continue
        rel_cols.append(_rebased_pct_on_common_dates(ps, common_dates))
