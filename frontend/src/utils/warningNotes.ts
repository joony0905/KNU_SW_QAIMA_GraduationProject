// 백엔드 warning code → 사용자에게 보여줄 한국어 안내 매핑.
// AnalysisResultPanel 과 SettingPage(저장된 리포트) 가 함께 사용한다.
...
export function mapWarningsToNotes(warnings?: unknown[] | string[] | null): string[] {
  ...
  for (const raw of warnings) {
    ...
    // 매핑되지 않은 코드(NEWS_*:해시 등)는 사용자에게 노출하지 않는다.
    if (!matched) continue;
  }
