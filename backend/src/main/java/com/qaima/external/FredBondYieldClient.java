...
@Slf4j
@Component
public class FredBondYieldClient {
    ...
    public FredBondYieldClient(@Qualifier("fredWebClient") WebClient webClient) {
        this.webClient = webClient;
    }

    public Mono<java.util.List<FredObservationsResponse.Observation>> fetchMonthlyTreasurySeries(
            String seriesId,
            LocalDate from,
            LocalDate to
    ) {
        ...
        return webClient.get()
                ...
                .doOnNext(rows -> log.info(
                        "[FRED] monthly treasury series fetched. seriesId={}, from={}, to={}, rows={}",
                        seriesId,
                        from,
                        to,
                        rows.size()
