...
public class FinancialAdminService {
    ...
    public Mono<FinancialDto> create(String stockCode, FinancialDto dto) {
        return Mono.fromCallable(() -> stockRepository.findByStockCodeWithExchange(stockCode)
                ...
                .flatMap(entity -> Mono.fromCallable(() -> financialRepository.save(entity))
                        .subscribeOn(Schedulers.boundedElastic()))
    ...
    public Mono<FinancialDto> update(Long financialId, FinancialDto dto) {
        return Mono.fromCallable(() -> financialRepository.findById(financialId)
    ...
    public Mono<Void> delete(Long financialId) {
        return Mono.fromCallable(() -> financialRepository.existsById(financialId))
                .subscribeOn(Schedulers.boundedElastic())
                .flatMap(exists -> {
                    if (!exists) {
